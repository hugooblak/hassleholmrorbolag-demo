 ...
 * or photo. They call ha_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Hässleholm Rörbolag.
 *
 ...
 * @return string
 */
function ha_sample_note() {
	return ha_lead( 'ar_riktig' ) ? '' : ' <span class="ha-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ha_lead_img().
 */
function ha_lead_bild( $i ) {
	$bilder = (array) ha_lead( 'bilder', array() );
 ...
 * @return string
 */
function ha_ring_label() {
	$namn = ha_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ha_lead( 'telefon' ) ) : 'Ring ' . ha_lead( 'telefon' );
}
