<?php
/**
 * Hässleholm Rörbolag theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Hässleholm Rörbolag Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through ha_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'hassleholmrorbolag', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'hassleholmrorbolag' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'hassleholmrorbolag' ) ) );

		register_block_pattern_category( 'hassleholmrorbolag-sections', array( 'label' => __( 'Hässleholm Rörbolag sections', 'hassleholmrorbolag' ) ) );
		register_block_pattern_category( 'hassleholmrorbolag-pages', array( 'label' => __( 'Hässleholm Rörbolag page layouts', 'hassleholmrorbolag' ) ) );
	}
);
