...
add_filter(
	...
	function ( $exporters ) {
		$exporters['hassleholmrorbolag-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'hassleholmrorbolag-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'hassleholmrorbolag-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'hassleholmrorbolag-leads',
						'group_label' => __( 'Quote requests', 'hassleholmrorbolag-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['hassleholmrorbolag-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'hassleholmrorbolag-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
