...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'hassleholmrorbolag-leads' ),
					'singular_name' => __( 'Lead', 'hassleholmrorbolag-leads' ),
					'edit_item'     => __( 'Förfrågan', 'hassleholmrorbolag-leads' ),
					'search_items'  => __( 'Search leads', 'hassleholmrorbolag-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'hassleholmrorbolag-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'hassleholmrorbolag-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'hassleholmrorbolag-leads' ),
			'npl_phone'    => __( 'Phone', 'hassleholmrorbolag-leads' ),
			'npl_service'  => __( 'Needs', 'hassleholmrorbolag-leads' ),
			'npl_timeline' => __( 'When', 'hassleholmrorbolag-leads' ),
			'npl_zip'      => __( 'ZIP', 'hassleholmrorbolag-leads' ),
			'npl_source'   => __( 'Source', 'hassleholmrorbolag-leads' ),
			'npl_status'   => __( 'Status', 'hassleholmrorbolag-leads' ),
			'date'         => __( 'Received', 'hassleholmrorbolag-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'hassleholmrorbolag-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'hassleholmrorbolag-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'hassleholmrorbolag-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'hassleholmrorbolag-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'hassleholmrorbolag-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'hassleholmrorbolag-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'hassleholmrorbolag-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'hassleholmrorbolag-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'hassleholmrorbolag-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'hassleholmrorbolag-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'hassleholmrorbolag-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'hassleholmrorbolag-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
